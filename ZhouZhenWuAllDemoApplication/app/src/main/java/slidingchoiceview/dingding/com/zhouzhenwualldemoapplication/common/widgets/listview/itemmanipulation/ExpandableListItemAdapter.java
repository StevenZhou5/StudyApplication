package slidingchoiceview.dingding.com.zhouzhenwualldemoapplication.common.widgets.listview.itemmanipulation;
...
import slidingchoiceview.dingding.com.zhouzhenwualldemoapplication.common.widgets.listview.ArrayAdapter;
...
public abstract class ExpandableListItemAdapter<T> extends ArrayAdapter<T> {
	...
	private static final int DEFAULT_TITLE_PARENT_RESID = 10000; // 默认title父View的ResId
	private static final int DEFAULT_CONTENT_PARENT_RESID = 10001; // 默认Content父ViewPage的ResId
	...
	protected ExpandableListItemAdapter(Context context, List<T> items) {
		super(items);
		mContext = context;
		mTitleParentResId = DEFAULT_TITLE_PARENT_RESID;
		mContentParentResId = DEFAULT_CONTENT_PARENT_RESID;
	...
	private static class RootView extends LinearLayout {
		...
		private void init() {
			...
			mTitleViewGroup.setId(DEFAULT_TITLE_PARENT_RESID);
			addView(mTitleViewGroup);

			mContentViewGroup = new FrameLayout(getContext());
			mContentViewGroup.setId(DEFAULT_CONTENT_PARENT_RESID);
