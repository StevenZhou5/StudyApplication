...
def create_movie_feature_model(features, title_set, genres_set):
    ...
    # ===========进行全连接的神经网络搭建
    movie_final_layer = layers.Dense(256)(movie_concatenate)
...
# 不在用相乘在相加，而是直接用打平全连接的方式
def create_model_rate_predict(model_user_feature, model_movie_feature):
    # 最后整合模型：用用户特征与电影特征结合起来构成完整的评分模型
    # 先把两个链接起来
    layerConcatenate = layers.concatenate([model_user_feature.output, model_movie_feature.output])
    print('layerConcatenate:', layerConcatenate)

    # 然后构建全连接层
    final_rate_layer1 = layers.Dense(256)(layerConcatenate)
    final_rate_layer2 = layers.Dense(64)(final_rate_layer1)
    final_rate_layer = layers.Dense(1)(final_rate_layer2)

    # 构建最终的评分预测模型
    rate_predict_model = models.Model(inputs=[model_user_feature.inputs, model_movie_feature.inputs],
                                      outputs=final_rate_layer)

    rate_predict_model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=0.01), loss='mse')
...
def main():
    ...
    history = rate_predict_model.fit(
        ...
        train_targets_values, batch_size=256, epochs=5,
        validation_data=(
